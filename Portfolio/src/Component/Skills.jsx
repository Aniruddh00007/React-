      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {skillCategories.map((category, idx) => (
            ...
            >
              <h3 className="text-xl font-semibold mb-6 text-cyan-300">{category.title}</h3>
              <div className="flex flex-wrap justify-center gap-6">
                {category.skills.map((skill, index) => (
                  <div
                    key={index}
                    className="flex flex-col items-center text-sm font-medium text-white hover:text-cyan-400 transition-colors"
                  >
                    <div className="text-4xl drop-shadow-lg">{skill.icon}</div>
